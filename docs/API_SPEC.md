...
3. [Gateway API Endpoints (Admin/Configuration)](#3-gateway-api-endpoints-adminconfiguration)
   - [3.11 SSO Authentication (Enterprise)](#311-sso-authentication-enterprise)
...
### 3.11 SSO Authentication (Enterprise)

Public SSO endpoints do not require authentication. Admin SSO configuration endpoints require a JWT session with `settings:read` or `settings:write` permission.

#### `GET /api/v1/auth/oidc/authorize`

Initiates OIDC login. Redirects browser to the configured identity provider.

**Query Parameters:**
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `org_id` | UUID | Yes | Organization to sign in to |
...
OIDC callback. Verifies the `state` parameter against Redis (CSRF protection), exchanges the authorization code, provisions the user if needed, and redirects to the dashboard.

**Query Parameters:**
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `code` | string | Yes | Authorization code from IdP |
| `state` | string | Yes | State nonce generated by `/authorize` |

**Response:** `302 Redirect` to first configured `allowed_origins` URL.

**Errors:**
- `400 invalid_state` — State missing, expired, or already used
- `404 sso_not_configured` — OIDC not configured for this organization

---

#### `GET /api/v1/auth/saml/authorize`

Initiates SAML 2.0 login. Generates an AuthnRequest and redirects browser to the configured IdP.
...
**Response:** `302 Redirect` to IdP SSO URL with `SAMLRequest` and `RelayState`.

---

#### `POST /api/v1/auth/saml/acs`

SAML Assertion Consumer Service. Verifies `RelayState` against Redis (CSRF protection), parses the SAML response, provisions the user if needed, and redirects to the dashboard.
...
| `RelayState` | string | Yes | RelayState nonce generated by `/saml/authorize` |

**Response:** `302 Redirect` to first configured `allowed_origins` URL.

**Errors:**
- `400 invalid_relay_state` — RelayState missing, expired, or already used
- `400 saml_error` — SAML response parsing or validation failed
- `404 sso_not_configured` — SAML not configured for this organization
...
**Auth:** JWT session with `settings:read` permission. `auth.org_id` must equal `org_id`.
...
**Auth:** JWT session with `settings:write` permission. `auth.org_id` must equal `org_id`.
...
For SAML, use `provider_type: "saml"` and provide `entity_id`, `certificate`, and `sso_url` instead of OIDC fields.

**Response:** The created/updated SSO config object.

---

#### `DELETE /api/v1/organizations/{org_id}/sso/{provider_type}`
...
|------|------|-------------|
| `provider_type` | string | `saml` or `oidc` |
